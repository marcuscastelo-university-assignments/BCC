...
__global__
void kernel(int *matriz_d, int *min, int tam)
{
...
int main(int argc, char const *argv[])
{
    ...
    fscanf(stdin, "%d", &dimA_lin); // Lê a dimensão linha matriz A
    fscanf(stdin, "%d", &dimA_col); // Lê a dimensão coluna matriz A
    fscanf(stdin, "%d", &dimB_lin); // Lê a dimensão linha matriz B
    fscanf(stdin, "%d", &dimB_col); // Lê a dimensão coluna matriz B
    ...
    kernel<<<1,dimA_lin*dimA_col, 1, streamA>>>(A_d, minA_d, dimA_lin*dimA_col);

    // Encontra o mínimo da matriz B
    kernel<<<1,dimB_lin*dimB_col, 1, streamB>>>(B_d, minB_d, dimB_lin*dimB_col);
