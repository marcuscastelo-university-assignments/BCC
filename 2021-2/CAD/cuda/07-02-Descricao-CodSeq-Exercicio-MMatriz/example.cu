...
int main(int argc,char **argv){
    ...
    gpuErrchk(cudaMalloc(&vetA_d,TAM*(sizeof(int)))); 
    ...
    gpuErrchk(cudaMemcpy(vetA_d,vetA_h,TAM*(sizeof(int)), cudaMemcpyHostToDevice)); 

    escalar_h=VLR_ESCALAR;
    //Copia o conteúdo de escalar_h, lido do terminal, para a variável constante escalar_d, no device
    gpuErrchk(cudaMemcpyToSymbol(escalar_d,&escalar_h,sizeof(int)));
    ...
    gpuErrchk(cudaMemcpy(vetA_h,vetA_d,TAM*(sizeof(int)), cudaMemcpyDeviceToHost));
