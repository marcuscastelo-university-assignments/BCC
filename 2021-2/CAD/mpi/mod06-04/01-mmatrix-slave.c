...
{
    int root, world_rank, num_proc, count, row_color, column_color, parent_rank;
    int *send_buffer = 0;
    int a_oper, b_oper, result;
    ...
    //    fprintf(trace, "Rank %d => a_oper=%d   b_oper=%d result=%d \n", my_rank, a_oper, b_oper, result);
    //    fflush(0);

    //making the multiplication
    result = a_oper * b_oper;
    ...
    root = 0;
    count = 1;
    MPI_Reduce(&result, (int *)0, count, MPI_INT, MPI_SUM, root, intercomm);
